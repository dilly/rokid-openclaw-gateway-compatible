...
// ============================================================
import { createWsBridgeService } from "./ws-bridge-service.js";
...
export default function register(api) {
    ...
    logger.info(`[rokid-openclaw-bridge] api keys: ${Object.keys(api ?? {}).join(",")}`);
    if (typeof api.registerChannel === "function") {
        ...
        logger.warn(`[rokid-openclaw-bridge] api.registerChannel is NOT a function (typeof=${typeof api.registerChannel}); channel not registered.`);
    }
}
function buildChannelPlugin(logger) {
    return {
        id: CHANNEL_ID,
        meta: {
            id: CHANNEL_ID,
        ...
        config: {
            listAccountIds: (_cfg) => {
                ...
                logger.info(`[rokid-openclaw-bridge] listAccountIds called -> ids=${JSON.stringify(ids)}`);
                return ids;
            },
            resolveAccount: (_cfg, accountId) => {
                const acc = accountId != null
                    ...
                    : undefined;
                logger.info(`[rokid-openclaw-bridge] resolveAccount called -> accountId=${accountId ?? "(none)"} found=${Boolean(acc)}`);
        ...
        onboarding: {
            ...
            getStatus: async (ctx) => {
                ...
                const linkCode = pluginCfg.linkCode ?? "";
                const linkSecret = pluginCfg.linkSecret ?? "";
            ...
            configure: async (ctx) => {
                ...
                await prompter.note("This plugin connects OpenClaw to an external device over WebSocket.\n" +
                    "You only need the link code and link secret provided by your device.", "About rokid-openclaw-bridge");
                const currentCfg = cfg?.plugins?.entries?.[CHANNEL_ID]?.config ?? {};
                const linkCode = await prompter.text({
                    message: "Link code (provided by your device)",
                    placeholder: "e.g. ABC123",
                    initialValue: currentCfg.linkCode ?? "",
                    validate: (v) => (v.trim() ? undefined : "Link code is required"),
                });
                const linkSecret = await prompter.text({
                    message: "Link secret (provided by your device)",
                    placeholder: "e.g. mysecret",
                    initialValue: currentCfg.linkSecret ?? "",
                    validate: (v) => (v.trim() ? undefined : "Link secret is required"),
                });
        ...
        gateway: {
            startAccount: async (ctx) => {
                logger.info(`[rokid-openclaw-bridge] startAccount INVOKED -> accountId=${ctx?.account?.accountId ?? "(none)"}`);
